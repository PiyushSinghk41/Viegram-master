...
public class Open_photo extends AppCompatActivity implements View.OnClickListener {
    ...
    ImageView profile_image, post_image, like_view, menu_click_view;

    TextView badgeText, user_name, post_points, caption, userName, location_text, time_text, errorText;

    RelativeLayout badgeLayout, progress_layout, back, activity_layout, like, comment, repost, options;

    SharedPreferences preferences;

    RelativeLayout postLayout;

    RelativeLayout showTag;

    RelativeLayout menu_home;

    RelativeLayout menu_open_layout;

    RelativeLayout menu_close;

    RelativeLayout menu_profile;

    RelativeLayout menu_stat;

    RelativeLayout menu_follow;

    RelativeLayout menu_notifications;

    RelativeLayout menu_settings;

    RelativeLayout menu_search;

    RelativeLayout menu_ranking;

    RelativeLayout menu_camera;
    ...
    List<TimelinePost> photo_data = new ArrayList<>();
            ...
            public void onResponse(Call<API_Response> call, retrofit2.Response<API_Response> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (result.getMsg().equals("201")) {
                        ...
                        if (photo_data.get(0).getImageWidth().equals("") || photo_data.get(0).getImageHeight().equals("")) {
                            ...
                            if (!photo_data.get(0).getFileType().equals("image")) {
                                ...
                                try {
                                    ...
                                    loader.displayImage(photo_data.get(0).getPhoto(), player.thumbImageView, options);
                        ...
                        ImageLoader loader = ImageLoader.getInstance();
                        loader.displayImage(photo_data.get(0).getProfileImage() , profile_image , options);
                ...
                if (response.isSuccessful()) {
                    ...
                    if (msg.equals("201")) {
                        ...
                        if (!result.getTotal_posts().equals(Timeline.resultp.getPhotosDetails().getTotalPosts())) {
                            ...
                            editor.commit();
//    Ti
// meline.resultp.getProfile_details().setTotalPosts(result.getTotal_posts());
